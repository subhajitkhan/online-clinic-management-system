...
session_start();
  ...
  if(isset($_POST['search']) )
  {
    ...
    $sqlval="SELECT * FROM appointment where appid='$ami' or doc_name='$ami'  or doc_id='$ami' or dept='$ami' or pat_id='$ami'  or pat_name='$ami'   or app_date='$ami'  or app_time='$ami'  or day='$ami' ";
 ...
 elseif(isset($_POST['cancel']) )
  {
    $del=$_POST['appid'];
    $sqldelete="DELETE  FROM appointment where  appid='$del'  ";
    $resultw=mysql_query($sqldelete);


    $sql="SELECT * FROM appointment ORDER BY appid ASC ";
...
<div style="color: #09AD3D ;">
  <h2 id="head">Have a look on appointments <?php echo $obj->admin_name; ?> </h2>
</div>
...
<form method="POST">
  <div style="height: 200px;width: 700px;background-color: #bdc3c7;margin: auto; "><br>
    <label style="font-size: 20px;margin-left: 250px;color: red; ">Enter any 'field' to search</label>
    <br></br><br>
...
<br></br>
<table style="margin-left: 80px;font-size: 18px;" width="1200"  border="1" cellpadding="1" cellspacing="1">
<tr style="background-color: #FFD104  ;color: red; ">
  ...
  while ($show_data=mysql_fetch_assoc($result)) {
    ...
    $pat=$show_data['pat_id'];
    $sql_patphn="SELECT pat_phn FROM pat_registration WHERE pat_id = '$pat' " ;
    $result_patphn=mysql_query($sql_patphn);
    $obj_patphn=mysql_fetch_object($result_patphn);
    $patphn=$obj_patphn->pat_phn;
    ...
    echo "<td>".$show_data['app_time']."</td>";
      ...
      ?>  
             <td><button style="background-color:green;color: white; ">Not Visited</button></td>
      <?php       
         }
...
<form method="POST">
  <div style="height: 250px;width: 700px;background-color: #bdc3c7;margin: auto; "><br>
    ...
    <p class="sub">
      <input style="margin-left:250px;  " type="submit" name="cancel" id="sub" value="Cancel Appointment" >         
       </p>
